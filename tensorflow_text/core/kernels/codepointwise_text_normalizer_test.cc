...
// Parameterized tests specs for CodepointWiseTextNormalizer.
const std::vector<Spec>& GetTestSpecs() {
  static const std::vector<Spec>& v = *new std::vector<Spec>{
      ...
      // "\x41\xCC\x80" is the decomposition of U+00C0.
      {
...
TEST_P(TestNormalization, TestGetOffsets) {
  ...
  const auto codepointwise_text_normalizer =
      CodepointWiseTextNormalizerSingleton::GetInstance(
          spec.lower_case_nfd_strip_accents)
          .GetNormalizer();
  ...
  codepointwise_text_normalizer->NormalizeText</*kGetOffsets=*/true>(
  ...
  if (is_normalized_identical) {
    ...
    output_normalized_text = spec.input;
    output_normalized_offset_mapping.resize(output_normalized_text.size() + 1);
    std::iota(output_normalized_offset_mapping.begin(),
              output_normalized_offset_mapping.end(), 0);
...
TEST_P(TestNormalization, TestNoGetOffsets) {
  ...
  std::vector<int> output_normalized_offset_mapping;
  bool is_normalized_identical;
  codepointwise_text_normalizer->NormalizeText</*kGetOffsets=*/false>(
      spec.input, &is_normalized_identical, &output_normalized_text,
...
INSTANTIATE_TEST_SUITE_P(CodepointWiseTextNormalizerTest, TestNormalization,
