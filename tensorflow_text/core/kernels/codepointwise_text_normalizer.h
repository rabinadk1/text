...
#ifndef THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_CODEPOINTWISE_TEXT_NORMALIZER_H_
#define THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_CODEPOINTWISE_TEXT_NORMALIZER_H_
...
// The mask for storing the length of the normalized string in the pool.
static constexpr unsigned int kNormalizedStringLengthMask =
    (1 << kBitsToEncodeUtf8LengthOfNormalizedString) - 1;

// Maximum length of utf8 bytes of normalized strings.
static constexpr unsigned int kMaximumUtf8LengthOfNormalizedString =
    (1 << kBitsToEncodeUtf8LengthOfNormalizedString) - 1;

// The mask for storing the offset of the normalized string in the pool.
static constexpr unsigned int kNormalizedStringOffsetMask =
    ((0x7FFFFFFF ^ kIsNormalizedStringDifferentMask) ^
     (kMaximumUtf8LengthOfNormalizedString - 1));
...
// A trie-based codepoint-wise Unicode text normalizer.
class CodepointWiseTextNormalizer {
 public:
  // Creates an instance.
  //
  // Args:
  ...
  static absl::StatusOr<CodepointWiseTextNormalizer> Create(
      const uint32_t* trie_data, int data_for_codepoint_zero,
      const char* normalized_string_pool) {
    CodepointWiseTextNormalizer result;
  ...
  // in the original input it should map to.
  //
 ...
 private:
  ...
  static bool IsNormalizedStringDifferent(int data) {
    return static_cast<bool>(data & kIsNormalizedStringDifferentMask);
  }

  // Calls this only when IsNormalizedStringDifferent(data) returns true.
  absl::string_view GetNormalizedString(int data) const {
    const int len = data & kNormalizedStringLengthMask;
    if (!len) {
      return "";
    }
    const int offset = (data & kNormalizedStringOffsetMask) >>
                       kBitsToEncodeUtf8LengthOfNormalizedString;
...
#endif  // THIRD_PARTY_TENSORFLOW_TEXT_CORE_KERNELS_CODEPOINTWISE_TEXT_NORMALIZER_H_
