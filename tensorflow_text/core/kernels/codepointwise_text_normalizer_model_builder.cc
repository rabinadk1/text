...
#include "tensorflow_text/core/kernels/codepointwise_text_normalizer_model_builder.h"
...
#include "tensorflow_text/core/kernels/codepointwise_text_normalizer_model_generated.h"
...
BuildCodepointWiseTextNormalizerAndExportToFlatBuffer(
  ...
  const auto& text_normalizer =
      text_norm::CodepointWiseTextNormalizerSingleton::GetInstance(
  ...
  auto text_normalizer_config = CreateCodepointWiseTextNormalizerConfig(
      builder, lower_case_nfd_strip_accents, array,
      text_normalizer.GetDataForCodepointZero(), mapped_string_pool);
  builder.Finish(text_normalizer_config);
  return std::string(reinterpret_cast<const char*>(builder.GetBufferPointer()),
                     builder.GetSize());
}

/*static*/ absl::Status
CodepointWiseTextNormalizerSingleton::BuildCodepointWiseTextNormalizer(
  ...
  for (const char32 cp : AllValidUnicodeCodePoints()) {
    ...
    if (cp_norm != cp_view) {
      ...
      data |= kIsNormalizedStringDifferentMask;
      ...
      if (!cp_norm.empty()) {
        ...
        data |= cp_norm.size();
        data |= (current_offset << kBitsToEncodeUtf8LengthOfNormalizedString);
      }
    }
    ...
    if (data == 0) {
      // Skip if nothing is stored in the encoded data.
      continue;
    }
...
CodepointWiseTextNormalizerSingleton::CodepointWiseTextNormalizerSingleton(
    bool lower_case_nfd_strip_accents) {
  auto status = BuildCodepointWiseTextNormalizer(
      lower_case_nfd_strip_accents, trie_data_, data_for_codepoint_zero_,
      mapped_value_pool_);
  if (!status.ok()) {
    ...
    LOG(ERROR) << "Unexpected error. Failed to build the data for "
                  "CodepointWiseTextNormalizer. Error message: "
               << status.error_message();
    return;
  }
  auto char_set_recognizer_mapper = CodepointWiseTextNormalizer::Create(
  ...
  if (!char_set_recognizer_mapper.ok()) {
    ...
    LOG(ERROR) << "Unexpected error: Failed to initialize "
                  "CodepointWiseTextNormalizer from the data.";
    return;
  }
  char_set_normalizer_ = absl::make_unique<CodepointWiseTextNormalizer>(
